...
- **One tab per conversation, with its own logins.** Steel open source runs one Chrome per
  ...
  right tab. Up to `GTWY_BROWSER_MAX_TABS` (default 10) conversations can hold a tab at once; beyond
  that, a conversation whose tab has been idle past `GTWY_BROWSER_IDLE_TIMEOUT_SECONDS` (default
  300 s) is recycled, and if none are idle the caller gets a clear "all tabs are in use" error.
...
GTWY_BROWSER_HANDOFF_TIMEOUT_SECONDS=900
GTWY_BROWSER_SNAPSHOT_MAX_CHARS=15000
```
...
5. Two conversations at once: send the same request under two different `thread_id` values. Both
   ...
   logged-out site. With `GTWY_BROWSER_MAX_TABS` reached and no idle tab, the caller gets
