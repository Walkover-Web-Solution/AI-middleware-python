...
async def yield_openrouter_response(client, configuration, service, count, execution_time_logs, timer, token_calculator):
    """
    Yield OpenRouter response chunks for API streaming.
...
async def openrouter_modelrun(configuration, apiKey, execution_time_logs, bridge_id, timer, message_id=None, org_id=None, name="", org_name="", service="", count=0, token_calculator=None, response_format=None, stream=False):
    try:
        ...
        if is_api_stream:
            ...
            timer.start()
            return yield_openrouter_response(
                openAI,
                configuration,
                service,
                count,
