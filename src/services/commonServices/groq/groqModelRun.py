...
async def stream_groq_response(client, configuration, response_format, execution_time_logs, service, count):
    """
    Stream Groq response. If tool call is detected, return full response without streaming.
...
async def yield_groq_response(client, configuration, service, count, execution_time_logs, timer, token_calculator):
    """
    Yield Groq response chunks for API streaming.
...
async def groq_runmodel(configuration, apiKey, execution_time_logs, bridge_id, timer, message_id, org_id, name="", org_name="", service="", count=0, token_calculator=None, response_format=None, stream=False):
    try:
        ...
        if should_stream:
            ...
            result = await stream_groq_response(
                groq_client,
                configuration,
                response_format,
                execution_time_logs,
