...
async def AiMlImageModel(configuration, apiKey, execution_time_logs, timer, images):
    try:
        ...
        generation_data, _ = await fetch(
            url='https://api.ai.ml/v1/generation/images',
            method='POST',
            headers=headers,
            json_body=configuration
        )
        
        execution_time_logs.append({"step": "AI.ml image generation time", "time_taken": timer.stop("AI.ml image generation time")})
        ...
        media_data, _ = await fetch(
            url=f'https://api.ai.ml/v1/media/images/{image_path}',
            method='GET',
            headers=headers
        )
        
        execution_time_logs.append({"step": "AI.ml fetch image URL time", "time_taken": timer.stop("AI.ml fetch image URL time")})
        ...
        execution_time_logs.append({"step": "AI.ml image Processing time", "time_taken": timer.stop("AI.ml image Processing time")})
