...
from src.services.billing.billing_utils import build_embed_hit_fee_event, build_llm_usage_event
...
async def make_request_data_and_publish_sub_queue(parsed_data, result, params, thread_info=None):
    ...
    # The flat per-hit fee for embed traffic, on top of the model cost and its
    # commission. Conditions, both read from state that already exists:
    #   * payer["is_embed"] — from billing_attribution, i.e. the FIRST agent's
    #     folder, so a nested agent outside an embed folder still counts as part
    #     of the embed hit that started the run;
    #   * this frame actually spent wallet money — either it ran on the platform
    #     key, or it burned tokens there before falling back to the customer's
    #     own key. An embed user paying with their own key owes nothing.
    # Not gated on a non-zero model cost: a cache hit is still a hit.
    #
    ...
    if payer["is_embed"] and (parsed_data.get("wallet") or parsed_data.get("_wallet_primary_cost")):
        fee_event = build_embed_hit_fee_event(billing_message_id, parsed_data.get("org_id"))
        if fee_event:
