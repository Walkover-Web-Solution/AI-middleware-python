...
class OpenaiCompletion(BaseService):
    async def execute(self):
        ...
        if self.type == 'image':
            ...
            conversation = ConversationService.createOpenaiCompletionConversation(self.configuration.get('conversation'), self.memory).get('messages', [])
            ...
            if len(model_response.get('choices', [])[0].get('message', {}).get("tool_calls", [])) > 0:
                functionCallRes = await self.function_call(self.customConfig, service_name['openai_completion'], gemini_response, 0, {})
