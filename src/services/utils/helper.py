...
class Helper:
    @staticmethod
    def encrypt(text):
        """Encrypt text using AES with configured key/IV."""
    ...
    def decrypt(encrypted_text):
        """Decrypt ciphertext generated by `encrypt`, handling multiple modes."""
        token = None
    ...
    def mask_api_key(key):
        """Mask the middle characters of an API key for safe display."""
        if not key:
            return ''
        if len(key) > 6:
    ...
    def update_configuration(prev_configuration, configuration):
        """Merge user-provided configuration into stored defaults."""
    ...
    def replace_variables_in_prompt(prompt, Aviliable_variables):
        """Substitute available variables into a prompt and collect misses."""
        missing_variables = {}
    ...
    def parse_json(json_string):
        """Try to parse a JSON string and return success metadata."""
        try:
    ...
    def get_value_from_location(obj, location):
        """Traverse a dotted path (with index support) on a nested object."""
        try:
            keys = location.split(".")
    ...
    def generate_token(payload, accesskey):
        """Create a JWT from the given payload using the access key."""
        return jwt.encode(payload, accesskey)

    async def response_middleware_for_bridge (service, finalResponse, isBridgeUpdated = False):
        """Normalise a bridge response and optionally broadcast RTLayer updates."""
        try:
    ...
    def find_variables_in_string(prompt):
        """Extract double-curly variable placeholders from a prompt."""
        variables = re.findall(r'{{(.*?)}}', prompt)
        return variables
    
    async def create_service_handler(params, service):
        """Instantiate the appropriate response handler for a service."""
        class_obj = None
    ...
    def calculate_usage(model, model_response, service):
        """Calculate token counts and expected cost from a model response."""
        usage = {}
        token_cost = {}
        permillion = 1000000
        modelObj = model_config_document[service][model]
    ...
    async def create_service_handler_for_batch(params, service):
        """Build a batch-processing handler for the given service."""
        class_obj = None
        if service == service_name['openai']:
    ...
    async def embedding_service_handler(params, service):
        """Return an embedding handler instance for supported services."""
        class_obj = None
        if service == service_name['openai']:
    ...
    def add_doc_description_to_prompt(prompt, rag_data):
        """Append document metadata to a prompt for retrieval-aware context."""
    ...
    def append_tone_and_response_style_prompts(prompt, tone, response_style):
        """Add tone/response-style modifiers to the prompt when supplied."""
        if tone:
    ...
    async def get_timezone_and_org_name(org_id):
        """Look up timezone/org-name data via cache or external API."""
    ...
    def sort_bridges(bridges, metrics_data):
        """Order bridges by available metrics, preserving untouched entries last."""
    ...
    def get_current_time_with_timezone(tz_identifier):
        """Return the hour/minute offset for a timezone identifier."""
        try:
    ...
    def get_req_opt_variables_in_prompt(prompt, variable_state, variable_path):
        """Classify prompt variables as required or optional using state metadata."""
        def flatten_values_only(d):
            """Flatten nested dict values, ignoring top-level keys."""
            result = {}
        ...
        def flatten_dict(d, parent_key=''):
            """Recursively flatten keys into dotted paths marked required."""
            flat = {}
            for k, v in d.items():
