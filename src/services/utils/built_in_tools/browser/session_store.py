...
def idle_timeout() -> int:
    return int(getattr(Config, "GTWY_BROWSER_IDLE_TIMEOUT_SECONDS", 300) or 300)


def handoff_timeout() -> int:
    return int(getattr(Config, "GTWY_BROWSER_HANDOFF_TIMEOUT_SECONDS", 900) or 900)


def max_tabs() -> int:
    return max(1, int(getattr(Config, "GTWY_BROWSER_MAX_TABS", 10) or 10))


def tab_idle_limit(tab: dict) -> int:
    return handoff_timeout() if (tab or {}).get("handoff_active") else idle_timeout()


def registry_ttl() -> int:
    return max(1800, max(idle_timeout(), handoff_timeout()) * 6)
...
async def save_registry(registry: dict) -> None:
    await store_in_cache(REGISTRY_KEY, registry, ttl=registry_ttl())
