...
def cdp_ws_url() -> str:
    ...
    override = getattr(Config, "STEEL_CDP_URL", None)
    if override:
        return override
...
def public_debug_url(debug_url: str | None) -> str | None:
    """Rewrite Steel's debugUrl host to STEEL_PUBLIC_URL when set.

    Steel builds debugUrl from its own DOMAIN env var. When that is an internal
    hostname the user's browser cannot reach, STEEL_PUBLIC_URL lets gtwy hand
    out a reachable one instead.
    """
    if not debug_url:
        return None
    public = getattr(Config, "STEEL_PUBLIC_URL", None)
    if not public:
        return debug_url
    pub = urlparse(public)
    orig = urlparse(debug_url)
    return urlunparse((pub.scheme or orig.scheme, pub.netloc, orig.path, orig.params, orig.query, orig.fragment))
...
def live_view_url(debug_url: str | None, target_id: str | None) -> str | None:
    ...
    """
    base = public_debug_url(debug_url)
    if not base:
        return None
    if not target_id:
        return base
