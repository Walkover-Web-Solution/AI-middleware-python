...
def _build_memory_id(thread_id: str, sub_thread_id: str, bridge_id: str, version_id: Optional[str]) -> str:
    version_or_bridge = (version_id or bridge_id or '').strip()
    return f"{thread_id.strip()}_{sub_thread_id.strip()}_{version_or_bridge}"


async def get_gpt_memory(bridge_id: str, thread_id: str, sub_thread_id: str, version_id: Optional[str] = None) -> Tuple[str, Optional[Any]]:
