...
async def load_model_configuration(model, configuration, service):
    ...
    for key, config in model_config.items():
        if key == 'type' or key == 'specification':
            continue
        if "level" in config and config["level"] == 2 or key in configuration:
...
async def process_background_tasks(parsed_data, result, params, thread_info):
    asyncio.create_task(create([parsed_data['usage']], result["historyParams"], result['response'], parsed_data['version_id'], thread_info))
