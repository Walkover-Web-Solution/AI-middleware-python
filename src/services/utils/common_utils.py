...
async def process_background_tasks_for_error(parsed_data, error):
    ...
    tasks = [
        ...
        send_error_to_webhook(parsed_data['bridge_id'], parsed_data['org_id'], str(error), error_type='general_exception', bridge_name=parsed_data.get('name'), is_embed=parsed_data.get('is_embed'), user_id=parsed_data.get('user_id'))
    ]
