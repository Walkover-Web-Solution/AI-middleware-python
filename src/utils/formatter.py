...
def render_template_to_html(card_json_template, variables_data={}):
    ...
    # Identify root key
    root_key = next((k for k in cloned_template.keys() if k.startswith("template_")), None)
    target_node = cloned_template[root_key] if root_key else cloned_template
    
    # Apply variables
    apply_variables(target_node, variables_data)
    
    # Render
    return render_node(target_node)
