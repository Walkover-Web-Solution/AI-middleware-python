...
async def create_version(request):
    try:
       ...
       if bridge_data is None:
           return Json_response(status_code=400, success = False,message="no version found", data=None)
...
async def publish_version(request, version_id):
    try:
        ...
        if result['success']:
            return Json_response(status_code=200, success = True,message= "version published successfully", data= {"version_id": version_id})
        return result
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=e)
...
async def discard_version(request, version_id):
    ...
    if 'success' in result:
        return Json_response(status_code=200, success = True,message= "version changes discarded successfully", data= {"version_id": version_id})
    return result
