...
async def auth_and_rate_limit(request: Request):
    """Authenticate chatbot requests and enforce per-bot rate limits."""
...
async def public_auth_and_rate_limit(request: Request):
    """Validate public agent access and throttle repeated calls."""
...
async def combined_auth(request: Request):
    """Try chatbot auth first and fall back to public agent checks if needed."""
    try:
...
async def send_message(request: Request, botId: str):
    """Forward a user message to the specified chatbot instance."""
...
async def reset_chat(request: Request, botId: str):
    """Reset the chat history for the chatbot identified by `botId`."""
